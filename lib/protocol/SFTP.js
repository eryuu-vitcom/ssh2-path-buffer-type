...
class SFTP extends EventEmitter {
  ...
  open(path, flags_, attrs, cb) {
    ...
    */
    ...
    // vitcom : allow type buffer for path
    if (typeof path === "string")
      buf.utf8Write(path, p += 4, pathLen);
    else
      path.copy(buf, p += 4, 0, pathLen);
  ...
  unlink(filename, cb) {
    ...
    */
    ...
    // vitcom : allow type buffer for filename
    if (typeof filename === "string")
      buf.utf8Write(filename, p += 4, fnameLen);
    else 
      filename.copy(buf, p += 4, 0, fnameLen);
  ...
  rename(oldPath, newPath, cb) {
    ...
    */
    ...
    // vitcom : allow type buffer for oldPath
    if (typeof oldPath === "string")
      buf.utf8Write(oldPath, p += 4, oldLen);
    else 
      oldPath.copy(buf, p += 4, 0, oldLen);
    writeUInt32BE(buf, newLen, p += oldLen);
    // vitcom : allow type buffer for newPath
    if (typeof newPath === "string")
      buf.utf8Write(newPath, p += 4, newLen);
    else 
      newPath.copy(buf, p += 4, 0, newLen);
    ...
    */
    ...
    if (count !== undefined) {
      ...
      for (let i = 0; i < count; ++i) {
        ...
        // vitcom : return both UTF-8 filename and the original buffer
        // const filename = bufferParser.readString(true);
        const filenameBuffer = bufferParser.readString(false);
        ...
        names.push({ filename, filenameBuffer, longname, attrs });
      }
      if (names !== undefined) {
        sftp._debug && sftp._debug(
